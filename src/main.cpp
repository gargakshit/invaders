...
#pragma message("Building for emscripten without SDL threads")
#endif
#endif
...
struct RenderLoopArgs {
  ...
  SDL_Window *window;
  bool &paused;
  unsigned int &lastPartialFrame;
  GLubyte *displayFramebuffer;
  GLuint displayTexture;
  ImGuiIO &io;
  int *displayScale;
};

static inline void renderLoop(void *argsRaw) {
  auto args = static_cast<RenderLoopArgs *>(argsRaw);

  auto &done = args->done;
  auto &bus = args->bus;
  auto window = args->window;
  auto &paused = args->paused;
  auto &lastPartialFrame = args->lastPartialFrame;
  auto displayFramebuffer = args->displayFramebuffer;
  auto displayTexture = args->displayTexture;
  auto &io = args->io;
  auto displayScale = args->displayScale;
  ...
  if (!paused) {
    ...
    for (unsigned int x = 0; x < 224; ++x) {
      for (unsigned int y = 0; y < 32; ++y) {
        auto byte = bus.mem[0x2400 + ((x * 32) + y)];
...
int main(int argc, char **args) {
  // Only use the arguments when not running on emscripten
...
#ifdef __EMSCRIPTEN__
  if (!bus.LoadFileAt("space-invaders.rom", 0x0000)) {
#else
...
SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 0);
  ...
  auto *window =
      SDL_CreateWindow("Space Invaders", SDL_WINDOWPOS_UNDEFINED,
...
#ifdef __EMSCRIPTEN__
  emscripten_set_main_loop_arg(&renderLoop, &loopArgs, 0, 1);
